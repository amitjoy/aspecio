...
public final class AspecioGogoCommand {
  ...
  private void printInterceptorDescription(final String marker,
      final InterceptorDTO interceptorDescription) {
    final String shift = String.format("%" + (marker.length() + 3) + "s", "");
    System.out.println("  " + marker + " Service id " + interceptorDescription.serviceId
        + ", class " + interceptorDescription.interceptorClass.getName() + ", extra properties: "
    ...
    System.out.println(shift + "Provided by: " + bundle.getSymbolicName() + " "
        + bundle.getVersion() + " [" + serviceBundleId + "]");
  }
  ...
  public void woven(final String objectClassContains) {
    final List<InterceptedServiceDTO> interceptedServices =
        aspecio.getInterceptedServices(objectClassContains);
    printWoven(interceptedServices);
  }
