...
public final class AspecioServiceObject {
  ...
  public AspecioServiceObject(final ServiceScope serviceScope,
      final ServiceReference<?> originalRef, final Function<Object, Proxy> proxyFunction) {
    ...
    switch (serviceScope) {
      case PROTOTYPE:
        return new PrototypeServiceFactory<Proxy>() {
          @Override
          public Proxy getService(final Bundle bundle,
              final ServiceRegistration<Proxy> registration) {
          ...
          public void ungetService(final Bundle bundle,
              final ServiceRegistration<Proxy> registration, final Proxy service) {
      ...
      default:
        final Object originalService =
            originalRef.getBundle().getBundleContext().getService(originalRef);
        final Proxy instance = proxyFunction.apply(originalService);
        instance.setInterceptor(interceptor);
        instances.add(instance);
        return instance;
