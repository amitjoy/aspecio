...
public final class AspecioServiceController
  ...
  private final Map<ServiceReference<?>, ManagedWovenService> managedServices =
  ...
  @Override
  public void onAspectChange(final AspectInterceptorListener.EventKind eventKind,
      final String aspectName, final AspectInterceptor aspectInterceptor) {
    final List<WovenService> wovenServicesForAspect =
        serviceWeavingManager.getWovenServicesForAspect(aspectName);
  ...
  private synchronized void handleServiceArrival(final WovenService wovenService) {
    ...
    final ManagedWovenService old                 =
        managedServices.put(wovenService.originalReference, managedWovenService);
    if (old != null) {
      logger.warn("Got an old service that we shouldn't for service id {}",
          wovenService.originalServiceId);
      old.unregister();
    }

    final AspectInterceptorContext context = aspectInterceptorManager
        .getContext(wovenService.requiredAspects, wovenService.optionalAspects);
  ...
  private synchronized void handleServiceUpdate(final WovenService wovenService,
      final boolean requiredAspectsChanged, final boolean optionalAspectsChanged,
  ...
  public synchronized List<InterceptedServiceDTO> getInterceptedServices() {
    ...
    for (final ManagedWovenService mws : managedServices.values()) {
      final long bundleId =
          getLongValue(mws.wovenService.originalReference.getProperty(Constants.SERVICE_BUNDLEID));
